...
class Model
{
    ...
    function newcompagny($idmanager, $email, $name, $description, $contact_mail, $contact_phone) {
        
        $sql = "INSERT INTO Compagnies (id_manager, email, name, description, contact_mail, contact_phone) 
        VALUES (:idmanager, :email, :name, :description, :contact_mail, :contact_phone)";
                $stmt = $this->pdo->prepare($sql);
                $stmt->execute([
                    'idmanager' => $idmanager,
                    'email' => $email,
